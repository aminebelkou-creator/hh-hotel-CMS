...
## Phase 2: own domains, speed, forms — 25 September 2026

Finding 31: **the Makers CDN does not cache cloud-function responses**, whatever `Cache-Control` says: every request to a page shows `eo-cache-status: Cache Miss` even with `public, s-maxage=10`. Origin timing from Paris: ~280 ms for a request that touches no database, ~600 ms for one that runs two queries, ~1.1 s for a rendered page. Most of the time is database round trips (Neon's free tier also suspends after inactivity, so the first request after a pause takes 2–5 s). What helped without host-specific code: one SQL round trip per page instead of six Payload queries, and immutable releases cached in the process. Real edge caching needs either an edge function with the Cache API (per node, EdgeOne-specific) or the Cloudflare fallback, so it waits for Gate 1; the headers are already correct for either.

Finding 32: **the multi-tenant plugin refuses anonymous writes to a tenant field over REST by design** (`isValidTenantAssignment`: a userless write is allowed only through the Local API). A visitor's contact-form submission therefore goes through our own `POST /api/contact`, which validates the bot checks, reads the form, and stores the submission with the Local API under the form's tenant. The plugin's own REST create route is closed to visitors.

Finding 33: **a block named like a collection collides in GraphQL** (a `form` block against the `forms` collection made every GraphQL request fail with 500), the same as the earlier `rooms` collision; every block that might share a name sets `interfaceName` and `graphQL.singularName`.
