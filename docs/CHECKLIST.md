...
| Fix later | 11 | 1 | Small product gaps noticed in use, in a table at the end; started 25 Sep: legal set, offers on home, contact in header, Book button on phones, gallery and lighter photos, blog, guest reviews, publish guard and verification retry done the same day; video hero and AI blog drafts on 26 Sep |
| **Total** | **59** | **69** | |
...
| --- | --- | --- | --- |
| ~~Template changed in the admin, then "Publish site"~~ **Done 25 Sep** | The sidebar's Publish reads the **stored** site; an unsaved form change is ignored (owner hit this on 25 Sep: r7 shipped Maison after choosing Lumière). Workaround: Save, then Publish | The Website panel saves a site's unsaved settings before publishing (validates first; says "your changes were saved first"), shows "Unsaved changes" while the form is modified, and on a page with unsaved or draft changes tells the owner to publish the page first. Checked by `tests/visual/publish-guard.mjs` | 25 Sep |
...
| ~~Guest reviews and testimonials~~ **Done 25 Sep (by hand)** | `reviews` collection (per site: the guest's exact words and language, first name + initial, source Google/Booking/Tripadvisor/Expedia/direct, link, score and scale, month, draft/published) + `reviews` block (cards, stars or x/10 with a spoken label, `lang` on each quote, never translated; hidden while empty; no self-serving Review/AggregateRating markup). Customer zero: three real Google reviews sent by the owner (5/5), on the home page after the features. Still open: import from Google/Booking (own row) | 25 Sep |
...
| ~~Blog / news~~ **Done 25 Sep** | `posts` collection (per site, FR/EN, draft/published, cover photo, provenance; RLS row) + `news` block (latest cards, or every post on the blog page) → posts at `/<blog>/<post>` with BlogPosting data, in the sitemap; menu hides an empty blog. Generated sites get the home block and a blog page. Customer zero: *Le carnet du Marais*, four posts on the neighbourhood written from the hotel's confirmed page and checked public sources (museum and cathedral sites, Paris city), no prices. AI post drafts: own row below | 25 Sep |
| ~~AI blog drafts from the facts~~ **Done 26 Sep** | Every post was written by hand | Site → **Blog drafts** panel: pick a topic (practical info, services, rooms, breakfast) → a **draft** post written only from the confirmed facts, in the site's languages (`src/generate/post-drafts.ts`, `POST /api/sites/:id/suggest-post`). With a model configured it polishes the prose; any number no fact backs keeps the plain draft. Never published by itself; marked generated, turns human on the owner's first edit. When the facts are too thin it says which to confirm and writes nothing. Tested in `tests/int/generate.int.spec.ts` (draft only, tenant-bound, 404 across tenants) | 26 Sep |
...
| ~~Animated hero: static photo, video, or a slideshow~~ **Done 26 Sep (video)** | The hero is one static photo (Lumière adds the stars and booking bar); the licensed originals use video or sliding photos | Hero `videoUrl` / `videoMobileUrl`: a silent looping MP4 over the photo (`src/site/HeroVideo.tsx`), fetched only after page load, never with reduced motion or data saver, portrait cut on phones, fade-in, pause button (WCAG 2.2.2). Platform stock clip `public/stock/paris/` (Pexels, sources in `SOURCES.md`): the Seine and the Conciergerie, 1.7 MB desktop / 0.9 MB phone. Customer zero uses it over the lounge photo. `tests/visual/hero-video.mjs` checks play, cut, pause and reduced motion. Still open: slideshow option, owner upload of their own video | 25 Sep |
| ~~Gallery: a proper photo experience~~ **Done 25 Sep** | Lightbox on the native `<dialog>` (`src/site/Lightbox.tsx`, one small client component per page; plain links to the full-size file without JS): tap, swipe, arrow keys, Esc, caption from the alt text. **Lighter images on every block**: the release carries an index of the hotel's photos (`snapshot.images`: srcset from the 400 / 960 / 1920 WebP variants, intrinsic size); the renderer emits `srcset` + `sizes` + `width`/`height` on every platform photo, `fetchpriority="high"` on the first hero. Measured on customer zero at 390 px: 17 of 17 photos load the 400 px variant instead of the 1600 px original | Batch 3 (same day): photos stretched on phones (the width/height attributes without `height: auto`) fixed at the base; one page lightbox (`src/site/Lightbox.tsx`) now also opens text-and-image photos and, on the rooms page, each room's photo and thumbnails. Still open: video/slideshow hero (own row) | 25 Sep |
| ~~Publish verification right after a code deploy~~ **Done 25 Sep** | The post-publish check fetched the public page while the edge was still warming up after a deploy, got a 404 and refused r9 (correctly); r10 a minute later was fine | `verifyPublicPage` tries 4 times (waits 2, 4, 8 s) before failing the publish and rolling back; tested with a stand-in edge that answers 404 twice | 25 Sep |
