...
| ~~Gallery: a proper photo experience~~ **Done 25 Sep** | Lightbox on the native `<dialog>` (`src/site/Gallery.tsx`, a small client component; plain links to the full-size file without JS): tap, swipe, arrow keys, Esc, caption from the alt text. **Lighter images on every block**: the release carries an index of the hotel's photos (`snapshot.images`: srcset from the 400 / 960 / 1920 WebP variants, intrinsic size); the renderer emits `srcset` + `sizes` + `width`/`height` on every platform photo, `fetchpriority="high"` on the first hero. Measured on customer zero at 390 px: 17 of 17 photos load the 400 px variant instead of the 1600 px original | Still open: video/slideshow hero (own row) | 25 Sep |
