...
| Features, gallery, quote, text, FAQ | `.hh-features` (items may carry `.hh-feature-icon` with an inline SVG from the built-in set, `src/site/icons.tsx`), `.hh-gallery` (figures wrap `.hh-gallery-link`; the lightbox `.hh-lightbox` is a native dialog with `.hh-lb-close/-prev/-next` — base styles are dark and usually left alone), `.hh-quote`, `.hh-prose`, `.hh-faq`; `.hh-checklist` under text-and-image |
...
Images: every platform photo renders with `srcset` (400 / 960 / 1920 WebP variants), a `sizes` hint per context (`src/site/Img.tsx`) and its intrinsic `width`/`height`, so a template's CSS must set `height: auto` (or an explicit `aspect-ratio`) rather than rely on the attributes.
