...
| Features, gallery, quote, text, FAQ | `.hh-features` (items may carry `.hh-feature-icon` with an inline SVG from the built-in set, `src/site/icons.tsx`), `.hh-gallery` (figures wrap `.hh-gallery-link`; other viewable photos - text-and-image, room photo and thumbnails on the rooms page - are wrapped in `.hh-photo-link`; any link with `data-lightbox` opens in the page's one lightbox `.hh-lightbox`, a native dialog with `.hh-lb-close/-prev/-next` — base styles are dark and usually left alone), `.hh-quote`, `.hh-prose`, `.hh-faq`; hero video: `video.hh-hero-video` (absolute, `object-fit: cover`, `.is-playing` fades in) and `.hh-hero-video-toggle` (round pause button, bottom right) inside `.hh-hero--image`, both hidden under reduced motion; reviews: `.hh-reviews` section with `.hh-review-grid` of `figure.hh-review` (`.hh-review-rating`/`.hh-review-score`, `blockquote[lang]`, `.hh-review-author`, `.hh-review-meta`); blog: `.hh-news` section with `.hh-news-grid` of `.hh-news-card` (`.hh-news-media`, `.hh-news-date`, `.hh-news-title` with the stretched `.hh-news-link`, `.hh-news-excerpt`), post page `.hh-post` (`.hh-post-head`, `.hh-post-back`, `.hh-post-cover`, `.hh-post-body` in `.hh-prose`); `.hh-checklist` under text-and-image |
...
Images: every platform photo renders with `srcset` (400 / 960 / 1920 WebP variants), a `sizes` hint per context (`src/site/Img.tsx`) and its intrinsic `width`/`height`, so a template's CSS must never rely on the attributes for the rendered height: the base sets `.hh-theme img { height: auto }`; a template that sets `width` + `aspect-ratio` keeps it, and one that needs a fixed box sets `height` explicitly (e.g. `100%` inside a sized parent).
