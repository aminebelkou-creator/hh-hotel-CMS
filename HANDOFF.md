...
- Tests: 5 more in `tests/int/generate.int.spec.ts` (facts only, draft only, thin facts, unbacked numbers, tenant-bound + 404/400 over HTTP); `tests/visual/blog-drafts.mjs` drives the panel in Chrome (screenshots in `docs/screenshots/2026-09-26/`). 170 tests green locally. Video hero (batch 7) went live as **r16**; the live check passed (desktop plays and pauses, phone loads the 540×960 cut, reduced motion fetches nothing).
