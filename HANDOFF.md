...
**Changed (batch 8, 26 Sep: Quartier corrections, AI blog drafts)**
- Customer zero's *Quartier* page corrected after a source check: the Louvre is about half an hour on foot (was "under 20 minutes", together with Notre-Dame, which stays about 20 minutes); Victor Hugo's house is usually free but charged during some exhibitions (was "free"). `src/onboarding/replace-text.ts` replaces exact strings in a site's pages per locale and republishes the page (run with a JSON list, here `hotel-herse-dor.corrections-2026-09-26.json`); the same text is fixed in `hotel-herse-dor.pages.ts`. Live as **r17**, checked on `/quartier` and `/en/quartier`.
- AI blog drafts: Site → **Blog** panel (`src/admin/BlogDraftPanel.tsx`) → `POST /api/sites/:id/suggest-post {topic}` → `src/generate/post-drafts.ts`. Topics: practical (address, times, contact), amenities, rooms, breakfast. Each draft is built from confirmed facts only, in the site's default language and French/English when enabled; a configured model polishes it, and any model text with a number no fact backs is dropped for the plain draft (`hasUnbackedNumbers`). Always `status: draft`, provenance `gen:post:<topic>` (becomes human on the owner's first edit); too few facts → 422 with what to do, nothing written. Customer zero today: practical and amenities work; rooms and breakfast wait for room and breakfast facts.
- Tests: 5 more in `tests/int/generate.int.spec.ts` (facts only, draft only, thin facts, unbacked numbers, tenant-bound + 404/400 over HTTP); `tests/visual/blog-drafts.mjs` drives the panel in Chrome (screenshots in `docs/screenshots/2026-09-26/`). 170 tests green locally.
- Measured: CI green (`9233704`), deploy green (run 36206830513); live check without writing anything: bad topic 400, anonymous 401, the site's admin page 200. Video hero (batch 7) went live as **r16**; the live check passed (desktop plays and pauses, phone loads the 540×960 cut, reduced motion fetches nothing).
...
- Measured: CI green (`a138bf5`), deploy run 36182922290 green; Neon: `apply.ts --only=posts` wrote 4 posts, the blog page and the home news block (other blocks and both languages intact, checked), customer zero republished as **r13**. Live: `/blog` and the four posts 200, no distorted photo at 390 px, the cover opens in the lightbox. Screenshots `docs/screenshots/2026-09-25/blog-*.jpg`.
