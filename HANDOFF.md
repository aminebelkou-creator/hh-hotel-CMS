...
| 2 | OWN + ENG | First `deploy` run: Neon migration from CI works, Makers step rejected the token (finding 21). Owner creates a Makers API token (console, longest expiry) and stores it via the prompt `%TEMP%\hh-set-edgeone-token.ps1` or directly as the GitHub secret `EDGEONE_PAGES_API_TOKEN`; then re-run `deploy` | Deploys must not depend on one PC | Live site deployed from CI; time recorded |
...
| 4 | OWN + ENG | Custom-domain test on `site.ouilockers.fr`: owner adds it in the Makers console (project `hh-platform-poc` → Domains), creates the CNAME (and TXT if asked) at the registrar; then measure DNS → verification → certificate → first 200. Makers domains are not reachable via `teo` (finding 20) | Gate 1 evidence for custom domains | Timings in docs/04 |
