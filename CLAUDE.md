...
40. **Behaviour on public pages is a client component, never an inline `<script>`** (`src/site/Gallery.tsx`): React duplicates and hoists inline scripts and they do not run; the page already hydrates for the contact form, so a small `'use client'` component costs a few KB. Photos render through `src/site/Img.tsx` (srcset from `snapshot.images`), never a bare `<img>`.
