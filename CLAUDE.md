...
32. **Site looks follow the design contract** ([`docs/11-design-contract.md`](docs/11-design-contract.md)): CSS uses tokens only (`--hh-*`), template layout lives in `[data-template='<id>']` sections of `site.css`, fonts are self-hosted from `src/design/fonts` (never a font CDN), and every template must pass `tests/int/design.int.spec.ts`. A hotel's brand is resolved by `resolveTheme()`, which derives what could fail contrast; only a text/background pair can be refused.
33. **A hotel's domain is served by rewrite, never by URL**: `src/proxy.ts` rewrites non-platform hosts to `/h/<host>/…`, blocks `/admin`, `/api`, `/preview` and `/s` there, and refuses `/h` on the platform host. Only `verified`/`active` domains are served (`status` is super-admin only). Platform hosts come from `PLATFORM_HOSTS` (default: localhost and `*.edgeone.dev|cool`); set it when a real platform domain exists. Never read `x-forwarded-host` unless `TRUST_FORWARDED_HOST=true`.
34. **Visitors' contact forms post to `POST /api/contact`** (`src/forms/contactEndpoint.ts`), never to `/api/form-submissions`: the multi-tenant plugin refuses anonymous tenant writes over REST (finding 32). The endpoint stores through the Local API under the form's own tenant. It is the only `/api` path open on a hotel's domain (`src/proxy.ts`).
35. **Every block whose slug could match a collection sets `interfaceName` and `graphQL.singularName`** (`form` vs `forms`, `rooms`), or GraphQL fails with 500 (findings 33 and the earlier rooms collision).
36. **Public pages carry `Cache-Control: public, max-age=0, s-maxage=10, stale-while-revalidate=60`** from the proxy; the Makers CDN currently ignores it (finding 31). The publish verification fetches with `?verify=<id>` so it never reads a cached copy. Release snapshots are cached in memory (`src/releases/resolve.ts`): never mutate one; the loader hands out a per-request copy of `site`.
37. **A `payload.update` without `_status` on a page that has a newer draft version turns the page into a draft** (Payload bases the update on the latest version). Scripts and jobs that touch published pages pass `_status: 'published'` explicitly, or save with `draft: true`. Generation and translation always save with `draft: true` and never publish.
38. **Every model call goes through `src/ai/provider.ts`** (`getAi()`), never a vendor SDK. `AI_PROVIDER` unset means the mock: callers must have a deterministic path and tests script the provider with `setAiForTests`. Model output is data: keys are allow-listed, numbers no fact backs are dropped, facts it proposes are born unconfirmed.
39. **Generated blocks carry `provenance.sourceFact = gen:<page>:<slot>`**; regeneration rewrites only slots still `generated`. The pages `beforeChange` hook (`src/generate/protect.ts`) marks a block `human` when a signed-in person changes it; system writes set `req.context.generation`.
40. **Behaviour on public pages is a client component, never an inline `<script>`** (`src/site/Lightbox.tsx`): React duplicates and hoists inline scripts and they do not run; the page already hydrates for the contact form, so a small `'use client'` component costs a few KB. Photos render through `src/site/Img.tsx` (srcset from `snapshot.images`), never a bare `<img>`. Photos carry `width`/`height`: CSS must not let them set the rendered height (base `height: auto`; check phones at 390 px after any image change). A viewable photo is `<a data-lightbox="group" href="full">`; one `Lightbox` per page opens them.
