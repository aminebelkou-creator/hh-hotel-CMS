...
21. **Public routes live under `src/app/(sites)/s/[site]/`** (a root layout per site). Next.js page files may only export page things (`default`, `generateMetadata`, `dynamic`…); put helpers in `src/booking` or `src/releases`.
